...
class EventCreationTests(APITestCase):
    ...
    def _get_token(self, user):
        response = self.client.post(
            '/auth/token/',
            {'username': user.username, 'password': 'adminpass' if user.is_staff else 'regularpass'},
            format='json',
        )
...
class EventParticipantTests(APITestCase):
    ...
    def test_joining_same_event_twice_is_idempotent(self):
        token = self._get_token(self.user, 'alicepass')
        self.client.post(f'/events/{self.event.id}/participants/', HTTP_AUTHORIZATION=f'Token {token}')
        response = self.client.post(f'/events/{self.event.id}/participants/', HTTP_AUTHORIZATION=f'Token {token}')
